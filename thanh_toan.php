...
<body>
    ...
    <form method="POST" action="" class="check_out">
        ...
        <div class="info_container">
            <div class="info">
                ...
                <div class="pay">
                    <button type="submit" class="pay_button" id="submit_payment" name="submit_payment">Thanh toán</button>
                    <button type="submit" class="pay_button" id="submit_payment" name="submit_payment">Thanh toán M</button>
                </div>
            </div>
        </div>
    </form>
    ...
    document.getElementById('submit_payment').addEventListener('click', function(event) {
        event.preventDefault(); // Ngăn form submit mặc định

        const paymentMethod = document.querySelector('input[name="pttt"]:checked').value;

        if (paymentMethod === '1') { // Nếu chọn MoMo
            fetch('/create-payment', {
                ...
                body: JSON.stringify({
                    // Thông tin đơn hàng, số tiền, etc.
                    amount: 18000, // Lấy từ phần tổng tiền trong thực tế
                    orderInfo: 'Thanh toán đơn hàng Vòng tay hạt cườm xinh xắn (Mẫu 1)',
                    returnUrl: 'YOUR_RETURN_URL', // Thay bằng URL thật của bạn
                    notifyUrl: 'YOUR_NOTIFY_URL'  // Thay bằng URL thật của bạn
                })
            })
            ...
            document.querySelector('form').submit(); // Submit form nếu không phải thanh toán qua MoMo
        }
    });
...
app.post('/create-payment', (req, res) => {
    const { amount, orderInfo, returnUrl, notifyUrl } = req.body;
    ...
    const rawSignature = `partnerCode=${partnerCode}&accessKey=${accessKey}&requestId=${requestId}&amount=${amount}&orderId=${orderId}&orderInfo=${orderInfo}&returnUrl=${returnUrl}&notifyUrl=${notifyUrl}&requestType=${requestType}`;
